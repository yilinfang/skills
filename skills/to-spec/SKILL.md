...
Turn what has already been discussed into a **spec**: a standalone markdown file that a fresh session can read to understand the problem, the solution, and every decision settled so far. Synthesize.
...
1. **Slug.** If the argument is a bare name, use it as the issue slug; otherwise derive a kebab-case slug and confirm it with the user.
2. **Explore the codebase** until every module the spec names and every test it cites as prior art is a real name in the project. Use the project's existing names and conventions.
...
> Run `/to-tickets <slug>` to split it.
